...
function segfault()
    ...
    err = PPL.ppl_io_print_Coefficient(c[]) # This does not actually print anythin
    @test iszero(err)
    # See https://discourse.julialang.org/t/libc-file-for-stdout/22482
    err = PPL.ppl_io_fprint_Coefficient(Libc.FILE(RawFD(1), "w"), c[]) # Neither does this
    @test iszero(err)
    flush(stdout)
    println()
